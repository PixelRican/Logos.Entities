...
{
    internal sealed class EntityArchetypeCloningTest : IUnitTest
    {
        public void Run()
        {
            ComponentType[] argumentTypes = new ComponentType[10];

            AssertEntityArchetypeCloneConsistency(EntityArchetype.Create(0, 0));
            ...
            AssertEntityArchetypeCloneConsistency(EntityArchetype.Create(argumentTypes.AsSpan(0, 4), 0, 0));
            ...
            AssertEntityArchetypeCloneConsistency(EntityArchetype.Create(argumentTypes.AsSpan(0, 6), 0, 0));
            ...
            AssertEntityArchetypeCloneConsistency(EntityArchetype.Create(argumentTypes, 0, 0));
        }

        private static void AssertEntityArchetypeCloneConsistency(EntityArchetype archetype)
        {
            AssertWithComponentType(ComponentType.TypeOf<object>());
            AssertWithComponentType(ComponentType.TypeOf<Position2D>());
            AssertWithComponentType(ComponentType.TypeOf<Rotation2D>());
            AssertWithComponentType(ComponentType.TypeOf<Scale2D>());
            AssertWithComponentType(ComponentType.TypeOf<Matrix3x2>());
            AssertWithComponentType(ComponentType.TypeOf<Position3D>());
            AssertWithComponentType(ComponentType.TypeOf<Rotation3D>());
            AssertWithComponentType(ComponentType.TypeOf<Scale3D>());
            AssertWithComponentType(ComponentType.TypeOf<Matrix4x4>());
            AssertWithComponentType(ComponentType.TypeOf<Tag>());

            void AssertWithComponentType(ComponentType type)
            {
                AssertEntityArchetypeEquality(archetype, archetype.Clone(0, 0));

                EntityArchetype clone = archetype.CloneWith(type, 0, 0);
                ...
                if (index < 0)
                {
                    ...
                    AssertEntityArchetypeEquality(clone, EntityArchetype.Create(clone.ComponentTypes.AsSpan(), 0, 0));
                    AssertEntityArchetypeEquality(archetype, clone.CloneWithout(type, 0, 0));
                }
                else
                {
        ...
        private static void AssertEntityArchetypeEquality(EntityArchetype a, EntityArchetype b)
        {
            ...
            Debug.Assert(a.ChunkSize == b.ChunkSize);
            Debug.Assert(a.ChunkCapacity == b.ChunkCapacity);
